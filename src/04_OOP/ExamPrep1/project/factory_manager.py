...
class FactoryManager:
    ...
    def sell_products_to_store(self, store: BaseStore, *products: BaseProduct):
        ...
        filtered_products = [p for p in products if p.sub_type == store.sell]
    ...
    def statistics(self):
        ...
        for p in self.products:
            if p.model not in product_stat:
                product_stat[p.model] = 0
            product_stat[p.model] += 1

        result += (f"Unsold Products: {len(self.products)}. Total net price: "
                   f"{sum([p.price for p in self.products]):.2f}\n")

        ordered_products = sorted(product_stat)

        for model in ordered_products:
            result += f"{model}: {product_stat[model]}\n"
